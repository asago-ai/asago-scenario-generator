...
check "QA-SP1-STRUCT-04: No coupling to existing pipeline infrastructure" \
    bash -c '! grep -rn "asago_scenario_generator.pipeline.io\|asago_scenario_generator.manifest\|asago_scenario_generator.prompts" src/asago_scenario_generator/stpa/system_model/ || (echo "Import coupling found" && false)'

check "QA-SP1-STRUCT-05: Foundation models imported, not copied" \
    bash -c '
grep -rn "from asago_scenario_generator.stpa.models" src/asago_scenario_generator/stpa/system_model/ > /dev/null || { echo "No stpa.models imports found"; exit 1; }
grep -rn "from asago_scenario_generator.models.capability_profile import\|from asago_scenario_generator.models.risk_card import" src/asago_scenario_generator/stpa/system_model/ > /dev/null || { echo "No existing model imports found"; exit 1; }
