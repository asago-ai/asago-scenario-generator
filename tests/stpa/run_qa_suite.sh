...
from asago_scenario_generator.stpa import fixtures
print('All STPA modules importable')
"

check "QA-STRUCT-02: No coupling to existing pipeline infrastructure" \
    bash -c '! grep -rn "^from asago_scenario_generator.pipeline.io\|^import asago_scenario_generator.pipeline.io\|^from asago_scenario_generator.manifest\|^import asago_scenario_generator.manifest\|^from asago_scenario_generator.prompts\|^import asago_scenario_generator.prompts" src/asago_scenario_generator/stpa/infra/ || (echo "Import coupling found" && false)'
...
if [ -f src/asago_scenario_generator/stpa/models/capability_profile.py ]; then
    echo "Found capability_profile.py in stpa models - should not be copied"
    exit 1
fi
if [ -f src/asago_scenario_generator/stpa/models/risk_card.py ]; then
...
check "QA-FIX-03: All five required fixtures are present" \
    bash -c '
count=$(ls src/asago_scenario_generator/stpa/fixtures/*.yaml 2>/dev/null | wc -l | tr -d " ")
if [ "$count" -ne 5 ]; then
...
for f in loss_analysis_klarna.yaml capability_profile_klarna.yaml control_structure_klarna.yaml ica_enumeration_klarna.yaml enriched_threats_klarna.yaml; do
    if [ ! -f "src/asago_scenario_generator/stpa/fixtures/$f" ]; then
