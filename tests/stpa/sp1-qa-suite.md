...
**Steps:**
1. Verify `src/asago_scenario_generator/stpa/system_model/__init__.py` exists and is importable.
2. Verify the following SP1 modules exist and are importable:
   - `src/asago_scenario_generator/stpa/system_model/loss_analysis.py` (Stage 1a)
   - `src/asago_scenario_generator/stpa/system_model/profile.py` (Stage 1b)
   - `src/asago_scenario_generator/stpa/system_model/control_structure.py` (Stage 2)
   - `src/asago_scenario_generator/stpa/system_model/critic.py` (Completeness critic + revision)
   - `src/asago_scenario_generator/stpa/system_model/heuristics.py` (Structural heuristics)
   - `src/asago_scenario_generator/stpa/system_model/run.py` (Orchestration)
3. Verify `src/asago_scenario_generator/stpa/system_model/prompts/` directory exists.

**Command:**
```bash
uv run python -c "
from asago_scenario_generator.stpa.system_model import loss_analysis, profile, control_structure, critic, heuristics, run
...
**Steps:**
1. Verify all 14 required Jinja2 template files exist in `src/asago_scenario_generator/stpa/system_model/prompts/`.
...
for f in stage1a_system.j2 stage1a_user.j2 stage1b_system.j2 stage1b_user.j2 \
  ...
  if [ ! -f "src/asago_scenario_generator/stpa/system_model/prompts/$f" ]; then
...
```bash
uv run python -c "
from asago_scenario_generator.stpa.system_model.control_structure import RequirementSet, Requirement, ResponsibilitySet
...
**Steps:**
1. Grep `src/asago_scenario_generator/stpa/system_model/` for imports of `asago_scenario_generator.pipeline.io`, `asago_scenario_generator.manifest`, `asago_scenario_generator.prompts`.
2. Verify none of those imports exist (SP1 uses `stpa/infra/` clean copy).

**Command:**
```bash
grep -rn "asago_scenario_generator.pipeline.io\|asago_scenario_generator.manifest\|asago_scenario_generator.prompts" src/asago_scenario_generator/stpa/system_model/ || echo "No coupling found"
```

### QA-SP1-STRUCT-05: Foundation models imported, not copied

**Steps:**
1. Verify `src/asago_scenario_generator/stpa/system_model/` imports `LossAnalysis`, `ControlStructure` from `stpa/models/`.
2. Verify it imports `CapabilityProfile`, `Stage1Profile` from `asago_scenario_generator.models.capability_profile`.
3. Verify it imports `RiskCard` from `asago_scenario_generator.models.risk_card`.

**Command:**
```bash
grep -rn "from asago_scenario_generator.stpa.models" src/asago_scenario_generator/stpa/system_model/
grep -rn "from asago_scenario_generator.models.capability_profile import\|from asago_scenario_generator.models.risk_card import" src/asago_scenario_generator/stpa/system_model/
```
...
**Command:**
```bash
ruff check src/asago_scenario_generator/stpa/system_model/ tests/stpa/
```
