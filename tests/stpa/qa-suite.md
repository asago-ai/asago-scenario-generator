...
**Steps:**
1. Verify `src/asago_scenario_generator/stpa/__init__.py` exists and is importable.
2. Verify `src/asago_scenario_generator/stpa/infra/__init__.py` exists and is importable.
3. Verify `src/asago_scenario_generator/stpa/models/__init__.py` exists and is importable.
4. Verify the following infra modules exist and are importable:
   - `src/asago_scenario_generator/stpa/infra/llm.py`
   - `src/asago_scenario_generator/stpa/infra/call_log.py`
   - `src/asago_scenario_generator/stpa/infra/yaml_io.py`
   - `src/asago_scenario_generator/stpa/infra/templates.py`
   - `src/asago_scenario_generator/stpa/infra/manifest.py`
5. Verify the following model modules exist and are importable:
   - `src/asago_scenario_generator/stpa/models/loss_analysis.py`
   - `src/asago_scenario_generator/stpa/models/control_structure.py`
   - `src/asago_scenario_generator/stpa/models/ica_enumeration.py`
   - `src/asago_scenario_generator/stpa/models/enriched_threat_set.py`
   - `src/asago_scenario_generator/stpa/models/scenario_spec.py`
   - `src/asago_scenario_generator/stpa/models/scenario_envelope.py`
6. Verify `src/asago_scenario_generator/stpa/fixtures/` directory exists.

**Command:**
```bash
uv run python -c "
from asago_scenario_generator.stpa.models import loss_analysis, control_structure, ica_enumeration, enriched_threat_set, scenario_spec, scenario_envelope
from asago_scenario_generator.stpa.infra import llm, call_log, yaml_io, templates, manifest
...
**Steps:**
1. Grep `src/asago_scenario_generator/stpa/infra/` for imports of `asago_scenario_generator.pipeline.io`, `asago_scenario_generator.manifest`, `asago_scenario_generator.prompts`.
2. Verify none of those imports exist.

**Command:**
```bash
grep -r "asago_scenario_generator.pipeline.io\|asago_scenario_generator.manifest\|asago_scenario_generator.prompts" src/asago_scenario_generator/stpa/infra/ || echo "No coupling found"
```

### QA-STRUCT-03: Models imported from existing codebase, not copied

**Steps:**
1. Verify `src/asago_scenario_generator/stpa/` does not contain a `capability_profile.py` or `risk_card.py` model definition.
2. Verify the stpa models import `CapabilityProfile` and `RiskCard` from `asago_scenario_generator.models.capability_profile` and `asago_scenario_generator.models.risk_card`.

**Command:**
```bash
grep -r "from asago_scenario_generator.models.capability_profile import\|from asago_scenario_generator.models.risk_card import" src/asago_scenario_generator/stpa/
```
...
**Steps:**
1. For each fixture file in `src/asago_scenario_generator/stpa/fixtures/*.yaml`, check that the file begins with a YAML comment (line starting with `#`) documenting provenance.

**Command:**
```bash
for f in src/asago_scenario_generator/stpa/fixtures/*.yaml; do
  echo "=== $f ==="
  head -3 "$f"
done
```
...
**Command:**
```bash
ls src/asago_scenario_generator/stpa/fixtures/*.yaml
```
...
**Steps:**
1. Verify `LLMClient` and `LLMResult` are importable from `asago_scenario_generator.stpa.infra.llm`.
...
**Steps:**
1. Verify `STPARunManifest` is importable from `asago_scenario_generator.stpa.infra.manifest`.
