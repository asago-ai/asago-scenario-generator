...
Checklist for methodically assessing the quality of asago-scenario-generator pipeline output. Usable by humans or AI workers. Work through each section in order — automated checks first, then per-scenario manual review, then batch-level judgment.

## Prerequisites

- Pipeline run completed successfully (N/N scenarios generated)
- Eval scorecard generated (`asago-scenario-generator eval`)
- Report generated (`asago-scenario-generator report`)
- Know the **capability profile**: active zones, entry points, system capabilities (tools, memory, HITL, multi-agent)

---

## 1. Automated Metrics (run `asago-scenario-generator eval`)
